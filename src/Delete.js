...
const loaddata=()=>
{
	axios.get("http://localhost:5000/Book_Keeping_System")
...
const Delrec=(id)=>
{
	axios.delete(`http://localhost:5000/Book_Keeping_System/${id}`)
         ...
         <tr className="row ml-2">
           ...
           <td className="col-2">{key.publisher}</td>
            <td className="col-2">{key.price}</td>
              <td  class="col-2 "> <button className="btn btn-outline-info" onClick={()=> {Delrec(key.id)}}><i class="fa-solid fa-trash-can"></i></button></td>
         </tr>
